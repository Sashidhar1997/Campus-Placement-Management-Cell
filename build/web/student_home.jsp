...
<div class="w3-content" style="max-width:2000px;margin-top:46px">
   ...
   <div>
           ...
           <% 
                try{
                    String q;
                   q="select cname,seats_offered,salary,about from company c,student s where (s.cgpa>=c.min_cgpa_btech or s.cgpa>=c.min_cgpa_mtech or s.cgpa>=c.min_cgpa_mca) and s.email='"+email+"' and c.cname not in (select cname from stu_comp where sid='"+sid+"') order by salary desc; ";
                 //   q = "select cname,seats_offered,salary,about from company c,student s where (s.stream ='"+stream+"' and s.cgpa>=c.min_cgpa_btech) or (s.stream ='"+stream+"' and s.cgpa>=c.min_cgpa_mca) or (s.stream ='"+stream+"'and s.cgpa>=c.min_cgpa_mtech) and s.email='"+email+"' and c.cname not in (select cname from stu_comp where sid='"+sid+"') order by salary desc; ";      
    ...
    <div class="card-body">
      ...
      <p class="card-text">Seats Offered:<%= rs.getInt("seats_offered")%></p>
      
      <p class="card-text">Salary:<%= rs.getInt("salary")%></p>
      <p class="card-text"><%= rs.getString("about")%></p>
      
      <form action="stu_accept_comp.jsp" method="GET" >
