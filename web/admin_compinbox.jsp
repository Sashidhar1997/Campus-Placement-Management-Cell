<!--
in Admin panel.... the inbox for the messages from all companies
-->
...
<div class="w3-content" style="max-width:2000px;margin-top:46px">
   ...
   <%    
    ...
    try{
        ...
        String q="select * from comp_to_admin where to_add='"+admin+"'";
        ResultSet rs=st.executeQuery(q);
        while(rs.next()){
     
                %><tr>
                    <td><%=rs.getString("from_add") %>[<%=rs.getString("at_time") %>]</td>
                    <td><%=rs.getString("message") %></td>
                    <td><form action="admin_msg_comp.jsp" method="GET">
                        <input type="hidden" name="admin" value="<%=rs.getString("to_add")%>">
                       <input type="hidden" name="cname" value="<%=rs.getString("from_add")%>">
